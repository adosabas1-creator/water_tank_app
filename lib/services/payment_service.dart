...
class PaymentService {
  ...
  Future<int> addPayment(Payment payment) async {
    ...
    return db.transaction((txn) async {
      ...
      final existing = await txn.query(
        ...
        whereArgs: [payment.syncId, payment.paymentKey.trim().isEmpty ? payment.syncId : payment.paymentKey.trim()],
        limit: 1,
      );
      ...
      final id = await txn.insert('payments', data);
  ...
  Future<double> getRemainingAmountForPurchaseInvoice(int invoiceId) async {
    ...
    final result = await db.rawQuery('''
      SELECT
        MAX(pi.total_amount - COALESCE(p.paid_amount, 0), 0) AS remaining
  ...
  Future<void> updatePayment(Payment payment) async {
    ...
    if (payment.id == null || payment.id! <= 0) {
      throw ArgumentError('رقم الدفعة غير صالح.');
    }
    ...
    await db.transaction((txn) async {
      final oldRows = await txn.query(
        'payments',
        columns: ['purchase_invoice_id', 'sync_id'],
        where: 'id = ? AND is_deleted = 0',
        whereArgs: [payment.id],
        limit: 1,
      );
      if (oldRows.isEmpty) {
        throw StateError('الدفعة غير موجودة أو محذوفة.');
      }
  ...
  Future<void> deletePayment(int id) async {
    ...
    await db.transaction((txn) async {
      ...
      if (invoiceId != null) {
        await _refreshPurchaseInvoiceStatus(txn, invoiceId);
      }
    });
  }
  ...
  void _validatePayment(Payment payment) {
    ...
    if (payment.purchaseInvoiceId != null && payment.purchaseInvoiceId! <= 0) {
      throw ArgumentError('رقم فاتورة الشراء غير صالح.');
    }
    if (payment.paymentKey.trim().isEmpty && payment.syncId.trim().isEmpty) {
      throw ArgumentError('معرف الدفعة غير صالح.');
    }
  }
  ...
  Future<void> _validateReference(
    ...
    final referenceTable =
        payment.paymentType == 'client_payment' ? 'clients' : 'suppliers';
    final reference = await db.query(
      referenceTable,
      columns: ['id'],
      where: 'id = ? AND is_deleted = 0',
      whereArgs: [payment.referenceId],
      limit: 1,
    );
    ...
    if (payment.purchaseInvoiceId != null) {
      final invoice = await db.query(
        'purchase_invoices',
        columns: ['id', 'supplier_id', 'total_amount', 'is_deleted'],
        where: 'id = ?',
        whereArgs: [payment.purchaseInvoiceId],
        limit: 1,
      );
      if (invoice.isEmpty || (invoice.first['is_deleted'] as num? ?? 0).toInt() == 1) {
        throw StateError('فاتورة الشراء غير موجودة أو محذوفة.');
      }
      if (payment.paymentType != 'supplier_payment') {
        throw StateError('لا يمكن ربط دفعة عميل بفاتورة شراء.');
      }
      if ((invoice.first['supplier_id'] as num).toInt() != payment.referenceId) {
        throw StateError('الفاتورة لا تنتمي إلى المورد المحدد.');
      }
    }
  }

  Future<void> _validateInvoicePaymentLimit(
    DatabaseExecutor db,
    Payment payment, {
    ...
    if (alreadyPaid + payment.amount > total + 0.000001) {
      throw StateError('لا يمكن تسجيل الدفعة: إجمالي الدفعات سيتجاوز قيمة الفاتورة.');
    }
  }
