...
class FillingOperationService {
  ...
  Future<void> updateOperation(FillingOperation operation) async {
    ...
    data['is_synced'] = 0;
    await db.update('filling_operations', data, where: 'id = ? AND is_deleted = 0', whereArgs: [operation.id]);
  }

  Future<void> deleteOperation(int id) async {
    PermissionService.requireManagementRole();
    final db = await _dbHelper.database;
    await db.update('filling_operations', {'is_deleted': 1, 'is_synced': 0, 'updated_at': DateTime.now().toIso8601String()}, where: 'id = ? AND is_deleted = 0', whereArgs: [id]);
  }
}
